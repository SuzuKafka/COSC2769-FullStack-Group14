...
const RegisterVendor = () => {
  ...
  useEffect(() => {
    if (registerStatus === 'succeeded') {
      navigate('/vendor/my-products', { replace: true });
    }
  ...
  const validationErrors = useMemo(() => {
    ...
    if (!businessAddress.trim() || businessAddress.trim().length < 5) {
      errors.businessAddress = 'Business address must be at least 5 characters.';
    }
    if (!profileImage) {
  ...
  const handleSubmit = (event) => {
    ...
    if (formValues.contactEmail.trim()) {
      formData.append('vendorContactEmail', formValues.contactEmail.trim());
    }
    if (profileImage) {
  ...
  return (
    <section style={containerStyle}>
      <h1 style={{ marginBottom: '1.5rem' }}>Register as Vendor</h1>
      <form onSubmit={handleSubmit} style={{ display: 'grid', gap: '1rem' }} encType="multipart/form-data">
        ...
        <div>
          ...
          {showErrors && validationErrors.businessAddress && (
            <p style={errorStyle}>{validationErrors.businessAddress}</p>
          )}
        </div>
